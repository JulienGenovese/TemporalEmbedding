...
class SyntheticTransactionDatasetCore(Build):
    ...
    def save(self, df: pd.DataFrame) -> Path:
        ...
        What it does:
            Resolves output path by experiment, writes CSV/parquet, logs key
            cardinality stats, and returns the final path.
        """
        noise_level = self.config.noise.noise_level
        out_path = self.config.output.path_for(self.experiment, noise_level)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        df_to_save = df.assign(noise_level=noise_level)
        ...
        logger.info(
            "Saved {:,} rows × {} cols → {} (experiment={}, noise_level={:.2f})",
            len(df),
    ...
    def generate_and_save(self) -> Path:
        """Build the dataset and persist it to CSV in one call."""
